...
* `effect()` gave confidence intervals centered near zero after a fit with the default (parametric) inference. With **fect** 2.4.6 its intervals come from the inference stored on the fit, `fit$vartype` (#37, #60, #75).
...
* **Dependency**: `fect (>= 2.4.6)`. Several **fect** 2.4.6 fixes also change gsynth results; see fect's NEWS.
