...
* **New: `ci.method` argument on `gsynth()`**. `"normal"` (default; Wald: point ± z × SE) or `"basic"` (reflected-pivot bootstrap interval, Davison and Hinkley 1997). The point estimate and SE are unaffected by the choice; the intervals and p-values differ. Use `"basic"` with `inference = "nonparametric"` (see `?gsynth`). New in fect v2.4.2.
