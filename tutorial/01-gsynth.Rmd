...
The `force` option specifies fixed effects: `"none"`, `"unit"`, `"time"`, or `"two-way"`. The number of factors `r` counts the factors on top of these fixed effects, so `r = 0` with `force = "two-way"` is the two-way fixed-effects model. Cross-validation (`CV = TRUE`) selects the number of factors within the range specified by `c(0, 5)`. A single `r` with `CV = TRUE` searches from `r` to 5, so the default `r = 0` also searches 0 to 5. From v1.5.0 the CV default is rolling-window with `k = 20` folds, `cv.prop = 0.1`, `cv.buffer = 1`. To reproduce the pre-v1.5.0 block-CV behavior in existing code, set `cv.method = "block"` and `k = 5` explicitly.
...
The output reports `sigma2` (error variance), `IC` (information criterion), and the cross-validation table `CV.out`, with one row per candidate `r` and its MSPE (mean squared prediction error). With the default `cv.rule = "1se"`, cross-validation picks the smallest `r` whose MSPE is within one standard error of the lowest MSPE.
...
`parallel = TRUE` (default) runs the bootstrap in parallel, and the cross-validation too on large panels. `cores` is the number of parallel R worker processes, not CPU cores. Set it explicitly when running on a shared machine; if `cores = NULL`, `gsynth` uses the number of available cores minus 2, at most 8. With a multithreaded BLAS (OpenBLAS, MKL, Accelerate), each worker may also start several threads. To avoid oversubscribing the machine, set, for example, `OPENBLAS_NUM_THREADS=1` before starting R.
...
`out$wgt.implied` (`N_co × N_tr`; rows are control units, columns are treated units) stores the implied weights of all control units for each treated unit. Unlike classical synthetic control, the weights can be both positive and negative. `wgt.implied` is `NULL` when the model has no factors (`r.cv = 0`).
...
A single state (Wisconsin): its observed turnout and its estimated counterfactual. The `"gap"` plot always shows the average effect over all treated units; to look at one unit, pass `id` to the `"counterfactual"` plot.
...
The `"missing"` plot shows the data used in estimation. Gray cells with red dots indicate observations removed due to insufficient pre-treatment periods. The matrix is available in `out$obs.missing` (`1` = treated, `2` = not under treatment, `3` = missing, `4` = removed, `5` = placebo).
