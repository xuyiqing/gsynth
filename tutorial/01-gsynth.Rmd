...
The point estimate and SE are identical to a `ci.method = "normal"` fit on the same bootstrap replicates; the intervals and p-values differ. Asymmetry of the basic interval relative to the Wald interval signals skewness in the bootstrap distribution. Under the nonparametric bootstrap, the p-values come from the percentile rule (twice the smaller share of bootstrap estimates on either side of zero), not from the basic interval, so near zero the two can disagree: at event time -15 the basic interval includes zero, but the p-value is 0.028.
