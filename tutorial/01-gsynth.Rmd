...
The `force` option specifies fixed effects: `"none"`, `"unit"`, `"time"`, or `"two-way"`. The number of factors `r` counts factors on top of these fixed effects, so with `force = "two-way"`, setting `r` to 0 gives the two-way fixed-effects model. Cross-validation (`CV = TRUE`) selects the number of factors within the range specified by `c(0, 5)`. A single `r` with `CV = TRUE` searches from `r` to 5, so a call without `r` also searches 0 to 5. From v1.5.0 the CV default is rolling-window with `k = 20` folds, `cv.prop = 0.1`, `cv.buffer = 1`. To reproduce the pre-v1.5.0 block-CV behavior in existing code, set `cv.method = "block"` and `k = 5` explicitly. Cross-validation and the bootstrap use random draws; `seed` fixes them, so rerunning the code in this manual with the same `parallel` setting gives the same results (the number of `cores` does not change them).
...
out <- gsynth(Y ~ D + X1 + X2, data = simdata,
              ...
              parallel = TRUE, cores = 16, seed = 02139)
```

The cross-validation results are stored in `out$CV.out`, with one row for each number of factors tried. It reports `sigma2` (error variance), `IC` (information criterion), and `MSPE` (mean squared prediction error), among other measures. `out$r.cv` is the number of factors chosen.

```{r sim1-cv}
out$CV.out[, c("r", "sigma2", "IC", "MSPE")]
out$r.cv
```

Cross-validation picks `r` by the rule in `cv.rule`. The default, `"1se"`, picks the smallest `r` whose MSPE is within one standard error of the lowest MSPE.
...
`type = "factors"` plots the estimated time-varying factors, one curve per factor, and `type = "loadings"` shows the distribution of unit-level loadings on each factor. With `force = "two-way"`, as here, both plots also show the fixed effects: the time effects as the curve labeled 0, and the unit effects in the panel labeled FE (`include.FE = FALSE` leaves them out):
...
- `ci.method = "normal"` (default): Wald intervals, $\hat{\theta} \pm z_{1-\alpha/2}\,\widehat{\mathrm{SE}}$. Symmetric around the point estimate. Works with every `inference`, and it is the only interval for `inference = "jackknife"`.
- `ci.method = "basic"`: reflected-pivot bootstrap interval (@DavisonHinkley1997, Sec. 5.2.1). Asymmetric; reads tail quantiles of the bootstrap distribution. Works with `inference = "parametric"` and `"bootstrap"`. With parametric inference, its p-values compare the estimate with the draws simulated under no effect, so they agree with the basic interval (**fect** 2.4.7). With `inference = "jackknife"`, `gsynth()` stops, because the jackknife gives a standard error but no bootstrap distribution. With $B < 1000$ replications the tail-quantile endpoints can be unstable; **fect** emits a warning and recommends `nboots = 1000` for publication-grade CIs.

`gsynth()` accepts only these two values. For percentile, BC, and BCa intervals on the alternative estimands (`att.cumu`, `aptt`, `log.att`), call `estimand(fit, type, ci.method = ...)` post-fit. For a jackknife fit, `estimand()` also allows only `ci.method = "normal"`, and you must give it for `att.cumu`, `aptt` and `log.att`, whose defaults are other methods.
...
`parallel = TRUE` (default) runs the bootstrap in parallel (and the cross-validation too, on large panels). Set `cores` explicitly when running on a shared machine; if `cores = NULL`, `gsynth` uses the number of available cores minus 2, at most 8. `cores` counts R worker processes, not CPU cores. With a multithreaded linear-algebra library (BLAS, such as OpenBLAS, MKL or Apple's Accelerate), each worker may also start several threads, so the workers together can ask for more threads than the machine has cores. To avoid this, set the library's thread count to 1 before starting R, for example `OPENBLAS_NUM_THREADS=1` for OpenBLAS.
...
The point estimate and SE are identical to a `ci.method = "normal"` fit on the same bootstrap replicates; the intervals and p-values differ. Asymmetry of the basic interval relative to the Wald interval signals skewness in the bootstrap distribution. With `ci.method = "basic"` under the nonparametric bootstrap, the p-values come from the percentile rule (twice the smaller share of bootstrap estimates on either side of zero), not from the basic interval, so near zero the two can disagree: at event time -15 the basic interval includes zero, but the p-value is 0.028.
...
- **Placebo region (event times $-2$ through $0$):** the confidence interval should cover zero. If it does, the test does not reject pre-treatment zero, which is consistent with the parallel-trends assumption.
...
`out$wgt.implied` (`N_co × N_tr`) stores the implied weights of all control units for each treated unit. Unlike classical synthetic control, the weights can be both positive and negative. It is `NULL` when cross-validation selects no factors (`r.cv = 0`).
...
The gap plot always shows the average effect over all treated states. For one state, the counterfactual plot shows its observed outcome and its estimated counterfactual (Wisconsin):

```{r turnout-ct-wi}
plot(out, type = "counterfactual", id = "WI", main = "Wisconsin")
```
...
The `"missing"` plot is drawn by **panelView** from the data given to `gsynth()`. It shows each unit's treatment status by period; white cells are missing observations. Units that `gsynth()` removed for having fewer than `min.T0` pre-treatment periods still appear in the plot. The status used in estimation is in `out$obs.missing`: `1` = under treatment, `2` = not under treatment (control units, and treated units before treatment), `3` = missing, `4` = unit removed, `5` = placebo period.
...
plot(out, type = "missing", xlab = "Year", ylab = "State",
     main = "Treatment Status", id = out$id[out$tr],
