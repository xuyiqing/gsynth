...
- **Changes that affect results.** Some calls now give different results than in v1.4.0; for example, the default call now chooses the number of factors by cross-validation. See "Changes that affect results" in the [Changelog](aa-changelog.Rmd).
- **fect 2.4.x catch-up.** 
...
**gsynth** v1.5.0 requires **fect** `>= 2.4.6`. Until both are on CRAN, install the development versions of both packages from GitHub:

```{r install-fect-dev, eval = FALSE}
# fect from the dev branch (>= 2.4.6)
devtools::install_github("xuyiqing/fect", ref = "dev")
```

```{r install-github, eval = FALSE, message = FALSE, warning = FALSE}
# gsynth from the dev branch (1.5.0)
devtools::install_github("xuyiqing/gsynth", ref = "dev")
```

Once **gsynth** 1.5.0 is on CRAN, the standard install will suffice:
...
| `r` | integer | Number of factors on top of the fixed effects: one number (with `CV = TRUE`, the search runs from `r` to 5) or a range `c(r.min, r.max)` |
...
| `weight`, `W.est`, `W.agg` | character | `weight`: averaging weight (the same as `W.agg`); `W.est`: weight for the model fit |
| `parallel` | logical or character | `TRUE` (default): bootstrap in parallel R worker processes (cross-validation too, on large panels); `FALSE`: none; `"boot"` or `"cv"`: only that step |
| `cores` | integer | Number of R worker processes, not CPU cores (default: available cores minus 2, at most 8) |
