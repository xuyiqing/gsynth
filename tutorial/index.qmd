...
- **Changes that affect results.**
  - A single `r` with `CV = TRUE` (the default) now searches `r` to 5, as documented. In v1.3.1--v1.4.0 it was fitted without a search, so the default call fitted no factors.
  - `weight` weights only the averaged effects, like `W.agg` (in v1.3.x--v1.4.0 it was ignored). Use `W.est` to weight the model fit.
  - Several **fect** 2.4.6 fixes also change results (for example, `wgt.implied`). See the Changelog chapter.

:::

## Installation

**gsynth** v1.5.0 requires **fect** `>= 2.4.6`. Until both are on CRAN, install the development versions from GitHub, **fect** first:

```{r install-fect-dev, eval = FALSE}
# fect from the dev branch (>= 2.4.6, with the fixes gsynth 1.5.0 needs)
devtools::install_github("xuyiqing/fect", ref = "dev")
```

```{r install-github, eval = FALSE, message = FALSE, warning = FALSE}
# gsynth from the dev branch
devtools::install_github("xuyiqing/gsynth", ref = "dev")
```

If you installed **fect** from `dev` earlier, reinstall it: builds from before the 2.4.6 fixes were merged also report version 2.4.6.

Once **gsynth** v1.5.0 is on CRAN, the standard install will suffice:
...
| `r` | integer | Number of factors on top of the fixed effects: one number (with `CV = TRUE`, CV searches `r` to 5) or a range `c(r.min, r.max)` |
| `CV` | logical | Cross-validation to select number of factors (default `TRUE`) |
...
| `weight`, `W.est`, `W.agg` | character | Weight column: `weight` (same as `W.agg`) weights the averaged effects; `W.est` weights the model fit |
| `parallel` | logical | `TRUE` (default): run the bootstrap in parallel R worker processes; `FALSE`: run everything in the current session |
| `cores` | integer | Number of parallel R worker processes (default: available cores minus 2, at most 8) |
