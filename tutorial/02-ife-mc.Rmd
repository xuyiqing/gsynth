...
out_mc <- gsynth(Y ~ D + X1 + X2, data = simdata,
                 ...
                 inference = "bootstrap", seed = 02139)
```
...
out_ife_pb <- gsynth(Y ~ D + X1 + X2, data = simdata,
                     ...
                     placeboTest = TRUE,
                     placebo.period = c(-2, 0), seed = 02139)
```
...
Here the test rejects, although `simdata` has no effect before treatment: with only five treated units, the nonparametric bootstrap can reject a correct model too often, so read the test with that in mind.
...
| `gsynth(..., r = 2)` with `CV = TRUE` (the default) | `fect::fect(..., r = c(2, 5))`: in **fect**, one `r` is fitted as given; leaving out `r` searches 0 to 5 in both packages (**fect** 2.4.7) |

Most other arguments (`CV`, `force`, `nboots`, `parallel`, `cores`, `seed`, `min.T0`, `alpha`, `normalize`, `tol`) carry over by name. `r` carries over when it is a range or when `CV = FALSE`.
