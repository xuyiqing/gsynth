...
Each call to one of these paths emits a one-time-per-session console message. The deprecation itself changes no results --- it is a signal, not a break. (Some v1.5.0 fixes do change results, for example the search over `r`; see the Changelog.)
...
| `weight = "col"` | `W.agg = "col"` |
| `r = k` (one number below 5, with `CV = TRUE`) | `r = c(k, 5)`. In **fect**, one number is a single candidate; `r = NULL` (the default from **fect** 2.4.6) searches 0 to 5 |
...
Most other arguments (`CV`, `force`, `nboots`, `parallel`, `cores`, `seed`, `min.T0`, `alpha`, `normalize`, `tol`) carry over by name. `r` carries over when it is a range or when `CV = FALSE`; see the table for a single `r` with `CV = TRUE`.
