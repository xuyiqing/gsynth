...
gsynth <- function(formula = NULL, data, # a data frame (long-form)
    ...
    ## weight (fect's W.agg). Fit weights go through W.est.
    if (!is.null(weight)) {
        if (!is.null(W.agg) && !identical(weight, W.agg)) {
            stop("`weight` and `W.agg` name different columns. ",
                 "`weight` sets the weights for averaging treatment ",
                 "effects (the same role as `W.agg`): give one of them, ",
                 "or the same column in both. ",
                 "Use `W.est` for weights in the model fit.",
                 call. = FALSE)
        }
        W.agg <- weight
    }
